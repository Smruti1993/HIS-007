...
export const Availability = () => {
  ...
  const getFutureAppointmentCount = (avail: DoctorAvailability) => {
      ...
      return appointments.filter(apt => {
          ...
          // Only count future dates or today
          if (apt.date < todayStr) return false;
          
          // Safe local date parsing to get correct day of week
          const [y, m, d] = apt.date.split('-').map(Number);
          const aptDateObj = new Date(y, m - 1, d);
          
          if (aptDateObj.getDay() !== avail.dayOfWeek) return false;
